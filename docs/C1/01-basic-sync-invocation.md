...
**一句话概括**：同步调用是AI应用的基础，适合简单问答和内容生成场景
**适用场景**：单次交互、原型开发、内容生成、数据处理
**关键参数**：temperature(0.7)、timeout(30s)、max_tokens(1000)
**常见误区**：参数配置不当导致响应质量差或超时
...
**同步调用**：程序发送请求后阻塞等待，直到AI模型完全处理完毕返回完整结果的调用方式。
...
```python
def basic_sync_call(model):
    """基础同步调用示例"""
    model_instance = init_model(model)
    
    # 直接字符串调用
    response = model_instance.invoke("写一首关于春天的诗")
    print(response)
...
**Q1**: 多个不同配置的模型如何管理？
**A1**: 可以为不同场景创建专门的初始化函数：

```python
# 创意写作专用配置
def init_creative_model():
    return init_chat_model(
...
# 技术文档专用配置
def init_technical_model():
    return init_chat_model(
        model="technical-model", 
        temperature=0.3,  # 更准确严谨
        max_tokens=1500
    )
```
...
def robust_call_with_retry(prompt, max_retries=3):
    for attempt in range(max_retries):
        try:
            ...
            model = init_chat_model(model_name, timeout=timeout)
