...
def pretty_print_ai_response_suffix(response, token):
    separator = "=" * 60
    # 技术信息
    print(f"\n{separator}")
    print("📊 技术详情:")
...
def stream_call(model):
    ...
    # 流式调用
    token = None
    for chunk in model.stream(["请写一首关于颜色的五言绝句"]):
        if chunk.usage_metadata:
            token = chunk.usage_metadata
        print(chunk.content, end='', flush=True)

    pretty_print_ai_response_suffix(chunk, token)
